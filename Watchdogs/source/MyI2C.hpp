...
#include "utility/time.hpp"
    ...
    uint8_t *ptr_buffer;
    inline static LPC_I2C_TypeDef * i2c[3] ={ LPC_I2C0, LPC_I2C1, LPC_I2C2};
class I2cInterface
{
 public:
...
class I2c final : public I2cInterface, protected Lpc40xxSystemController
{
 public:
  ...
  enum class SlaveState: uint32_t
  {
      kWriteAck                       = 0x60;
      kDataReceivedAckSend            = 0x80;
      kDataReceivedNotAck             = 0x88;
      kStopSlave                      = 0xA0;
      kReadAck                        = 0xA8;
      kSlaveWriteAckSend              = 0xB8;
      kSlaveWriteNotAck               = 0xC0;
      kLastWrite                      = 0xC8;
  };
...
static void Slave_Initialize(uint8_t port, uint8_t slaveAddr, uint8_t buffer[]){
    SelPort = port;
    switch(SelPort){   
    case 0:
    LPC_SC->PCONP |= (1<<7);
    LPC_IOCON -> P0_27 &= ~(0b111);
    LPC_IOCON -> P0_28 &= ~(0b111);
    LPC_IOCON -> P0_27 |= (0b101);
    LPC_IOCON -> P0_28 |= (0b101);
    LPC_IOCON -> P0_27 |= (0b1 << 10);
    LPC_IOCON -> P0_28 |= (0b1 << 10);
    RegisterIsr(I2C0_IRQn, Slave_ISR);
    break;
    case 1:
    LPC_SC->PCONP |= (1<<19);
    LPC_IOCON -> P0_0 &= ~(0b111);
    LPC_IOCON -> P0_1 &= ~(0b111);
    LPC_IOCON -> P0_0 |= (0b001);
    LPC_IOCON -> P0_1 |= (0b001);
    LPC_IOCON -> P0_0 |= (0b1 << 10);
    LPC_IOCON -> P0_1 |= (0b1 << 10);
    RegisterIsr(I2C1_IRQn, Slave_ISR);
    break;
    case 2:
    LPC_SC->PCONP |= (1<<26);
    LPC_IOCON -> P0_10 &= ~(0b111);
    LPC_IOCON -> P0_11 &= ~(0b111);
    LPC_IOCON -> P0_10 |= (0b010);
    LPC_IOCON -> P0_11 |= (0b010);
    LPC_IOCON -> P0_10 |= (0b1 << 10);
    LPC_IOCON -> P0_11 |= (0b1 << 10);
    LOG_INFO("Using Port 2\n");
    RegisterIsr(I2C2_IRQn, Slave_ISR);
    break;
    }
    //i2c[SelPort]-> CONCLR |= 0x6C;
    i2c[SelPort]-> CONSET |= (1<<2) | (1<<6);
    i2c[SelPort]-> ADR0 = slaveAddr<<1;
    i2c[SelPort]-> ADR1 = slaveAddr<<1;
    i2c[SelPort]-> ADR2 = slaveAddr<<1;
    i2c[SelPort]-> ADR3 = slaveAddr<<1;
    i2c[SelPort]-> MASK0 = 0x0;
    i2c[SelPort]-> MASK1 = 0x0;
    i2c[SelPort]-> MASK2 = 0x0;
    i2c[SelPort]-> MASK3 = 0x0;
    ptr_buffer = buffer;
