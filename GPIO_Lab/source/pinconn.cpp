...
void pincon::inactive(uint8_t port, uint8_t pin){
    //used to clear bit that controls the resistor
    //can be used independently to deactive resistor
    *pincon[port][pin] &= ~(0b11<<3);
}
void pincon::pullup(uint8_t port, uint8_t pin){
     inactive(port,pin);
    *pincon[port][pin] |= (0b1<<3); 
}
void pincon::pulldown(uint8_t port, uint8_t pin){
    inactive(port,pin);
    *pincon[port][pin] |= (0b1<<4);
}
void pincon::repeater(uint8_t port, uint8_t pin){
    inactive(port,pin);
    *pincon[port][pin] &= (0b11<<3); 
}
